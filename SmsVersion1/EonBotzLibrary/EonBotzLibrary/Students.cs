    ...
    public class Students
    {
        ...
        public void UPDATE_DATA()
        {
            ...
            using(cmd = new MySqlCommand("update student set lastname=@lname, firstname=@fname, middlename=@mname, age=@age, dateofbirth=@dob, placeofbirth=@pob," +
                "contactno=@contact, emailAddress=@emailAddress, gender=@gender, maritalstatus=@marital, citizenship=@citizen, religion=@religion, address=@address WHERE studentId=@id", conn))
            {
            ...
            using (cmd = new MySqlCommand("SELECT * from student", conn))
            {
                ...
                while (mdr.Read())
                {
                    dt.Rows.Add(mdr[0].ToString(), mdr[1].ToString(), mdr[2].ToString(), mdr[3].ToString(), mdr[4].ToString(), mdr[5].ToString(), mdr[6].ToString(), mdr[7].ToString()
                        , mdr[8].ToString(), mdr[9].ToString(), mdr[10].ToString(), mdr[11].ToString(), mdr[12].ToString(), mdr[13].ToString());
                }
            }
        }
        ...
        public void PassData()
        {
            ...
            using(cmd = new MySqlCommand("SELECT * FROM student WHERE studentId LIKE @id", conn))
            {
                ...
                if (mdr.HasRows)
                {
                    ...
                    emailAddress = mdr[8].ToString();
                    gender = mdr[9].ToString();
                    maritalstatus = mdr[10].ToString();
                    citizenship = mdr[11].ToString();
                    religion = mdr[12].ToString();
                    address = mdr[13].ToString();
                }
                mdr.Close();
                conn.Close();
            }
