...
namespace EonBotzLibrary
{
    ...
    public class studentSched
    {
        ...
        public void showMaxenrolled()
        {
            ...
            dt.Clear();
            using (cmd = new MySqlCommand("select a.schedid, a.subjectcode, a.subjectTitle,d.name,a.date,a.timeStart,a.timeEnd,a.maxStudent,a.status,e.lec,e.lab, e.totalunits,count(studentSchedID) " +
                "from schedule a left join Sectioning l on l.SectionCategoryID =3 " +
                "left join studentSched c on c.schedId regexp a.schedID " +
                "left join rooms d on d.roomId =a.roomId " +
                "left join subjects e on e.subjectCode = a.subjectCode   " +
                "group by a.schedId having count(c.schedID) < a.maxStudent", 
                conn))
            {
                mdr = cmd.ExecuteReader();
